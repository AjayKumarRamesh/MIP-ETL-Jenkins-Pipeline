...
object MipToMarketoInteractionMulti extends ETLFrameWork {
  ...
  def buildPayloadInteraction(transformedDF: DataFrame): DataFrame = {
    ...
    activityDF3 = transformedDF.filter(transformedDF("Activity_Type") === eventInteraction).map(row => {
      s"""{
         ...
         |      "activityDate":"${convert({row.getTimestamp(30).toString})}",
         |      "activityTypeId":${row.getInt(18)},
         |      "primaryAttributeValue": "${row.getString(21)}",
         |      "attributes":
         |          [
         |              {
         ...
         |                    "value": "${removeControlChar(row.getString(19))}"
         |                },
         |                {
    ...
    activityDF4 = transformedDF.filter(transformedDF("Activity_Type") === digitalInteraction).map(row => {
      s"""{
         ...
         |                    "value": "${removeControlChar(row.getString(33))}"
         |                },
         |                {
  ...
  def sendPost(payload: String): String = {
    ...
    val externalEndpoint = apiConProps.getProperty(PropertyNames.LeadEndpoint)
    ...
    try {
      ...
      val sqlData1 =
        s"""(WITH ranked_data AS (
           ...
           |LEAD_DESC, LEAD_NOTE, LEAD_SRC_NAME, SALES_CHANNEL_NAME, CONTACT_PHONE, REGION, SUB_SRC_DESC, a.CREATE_TS, EVENT_REF_ID, a.MIP_ACTIVITY_SEQ_ID, REFERRER_URL, ACTIVITY_CMPN_CD
           |FROM
           |MAP_MKTO.MCV_MKTO_CUSTOM_ACTIVITY a
           |LEFT JOIN
           |MAP_MKTO.MCT_MKTO_PERSON p on
           |p.MIP_SEQ_ID = a.MIP_SEQ_ID
           |WHERE p.STATUS_CODE = 'P' AND a.STATUS_CODE = 'U' AND a.STRENGTH IS NOT NULL AND CAMPAIGN_CODE !='' AND CAMPAIGN_CODE IS NOT NULL AND RIGHT(p.MIP_SEQ_ID, 1) IN ($partitionNo)
           |ORDER BY 2 asc),
           |etl_config_data AS (
           |SELECT
      ...
      if(count > 0) {
        ...
        /*Updated - 08-10-2021
        val columnMapping = """CAMPAIGN_CODE=Content_Campaign_Code,
                              |MKTO_LEAD_ID=Id,
                              |COUNTRY_CODE=Item_Country_Code,
                              |LANG_CODE=Item_Language_Code,
                              |ACTIVITY_NAME=Item_Name,
                              |ASSET_TYPE=Item_Type,
                              |ACTIVITY_URL=Item_URL,
                              |UUC_ID=Item_ID,
                              |DRIVER_CAMPAIGN_CODE=Driver_Campaign_Code,
                              |FORM_NAME=Form_Name,
                              ...
                              |CTRY_CODE=Person_Country_Code,
                              |IBM_GBL_IOT_CODE=GBL_IOT_CD,
                              |SUB_REGION_CODE=GBL_IMT_CD,
                              |CAMPAIGN_NAME=Content_Campaign_Name,
                              |LEAD_DESC=Lead_Description,
                              |LEAD_NOTE=Lead_Note,
                              |LEAD_SRC_NAME=Lead_Source,
                              |SALES_CHANNEL_NAME=Sales_Channel,
                              |CONTACT_PHONE=Person_Phone,
                              |ACTIVITY_TYPE_ID=Activity_Type,
                              |REGION=GBL_RGN_CD,
                              |WEB_PAGE_SRC=WEB_PAGE_SRC,
                              |SUB_SRC_DESC=SUB_SRC_DESC,
                              |CREATE_TS=Create_TS,
                              |EVENT_REF_ID=Event_Brief_ID,
                              |MIP_ACTIVITY_SEQ_ID=MIP_ACTIVITY_SEQ_ID""".stripMargin.replaceAll("\\s+", "")
        ...
        if (hasColumn(parsedJson, "errors")) {
          parsedJson.show(false)
          val errorDetails = parsedJson.select(explode(col("errors")).as("errors")).select("errors.*")
