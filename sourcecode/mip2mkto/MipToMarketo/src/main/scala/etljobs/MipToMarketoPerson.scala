    ...
    try {
      val sqlData =
        s"""(WITH ranked_data AS (
           ...
           |PREF_CODE_10A00, MIP_SEQ_ID, STUDENT_FLG, IBMER_FLG, STATE_CD, PREF_CODE_15S8X, SAP_CUST_NUM,DOM_CLIENT_ID,DOM_BUY_GRP_CTRY,ACCNT_KNOWN_IND,MAIN_IND_CD,FIRMO_GBL_EMP_CNT,FIRMO_GBL_TURN_OVER_USD,
           |EXCLD_PH, CTRY_COVERAGE_CLIENT_TYPE_DSCR FROM
